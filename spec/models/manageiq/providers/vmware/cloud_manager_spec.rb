describe ManageIQ::Providers::Vmware::CloudManager do
  before do
    @hostname = Rails.application.secrets.vmware_cloud.try(:[], 'hostname') || 'VMWAREVCLOUD_HOSTNAME'
    @port = (Rails.application.secrets.vmware_cloud.try(:[], 'port') || '1111').to_i
    @userid = Rails.application.secrets.vmware_cloud.try(:[], 'userid') || 'VMWAREVCLOUD_USERID'
    @password = Rails.application.secrets.vmware_cloud.try(:[], 'password') || 'VMWAREVCLOUD_PASSWORD'
